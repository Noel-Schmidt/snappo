        ...
        <div class="grid gap-6">
          ...
          <div class="grid gap-4 md:grid-cols-3">
            <div class="md:col-span-2 grid gap-2">
              ...
              <Input id="pattern" v-model="pattern" placeholder="e.g. (\\w+)@(\\w+\\.\\w+)" class="h-10" />
              <p v-if="error" class="text-xs text-red-400">{{ error }}</p>
            </div>
            <div class="grid gap-2">
              <Label>Flags</Label>
              <div class="grid grid-cols-3 gap-2 text-sm">
                <div class="flex items-center justify-between rounded-md border border-neutral-800 bg-neutral-950 px-3 py-2">
          ...
          <div class="grid gap-4 md:grid-cols-3">
            ...
            <div class="grid gap-2">
              ...
              <Input v-model="replacement" :disabled="!replaceMode" placeholder="Replacement (supports $1, $<name>)" class="h-10 disabled:opacity-50" />
              <div class="flex flex-wrap gap-2">
                <Button @click="run" :disabled="!pattern">Run</Button>
                <Button variant="outline" @click="clearAll" :disabled="!pattern && !text">Clear</Button>
                <Button variant="outline" @click="copyResult" :disabled="!pattern || !text">Copy result</Button>
              </div>
            </div>
          </div>
          ...
          <div class="flex flex-wrap items-center gap-3 text-sm text-neutral-300">
            <Badge variant="secondary" class="border-neutral-700 bg-neutral-800/60 text-neutral-200">Matches: {{ summary.count }}</Badge>
            <Badge variant="secondary" class="border-neutral-700 bg-neutral-800/60 text-neutral-200">Time: {{ summary.ms }} ms</Badge>
            <Badge v-if="named" variant="secondary" class="border-neutral-700 bg-neutral-800/60 text-neutral-200">Named groups</Badge>
          </div>
          ...
          <div class="grid gap-4 md:grid-cols-2">
            <div class="flex flex-col space-y-4">
              ...
              <div
                  ...
                  v-html="highlighted"
              />
              <p class="text-xs text-neutral-500">Matches are highlighted. Groups use thin underline.</p>
            </div>
              ...
              <div class="rounded-md border border-neutral-800 bg-neutral-950">
                <div class="max-h-64 overflow-auto text-sm">
                  <table class="w-full table-fixed border-collapse">
                    ...
                    <tbody>
                    <tr v-for="(m, i) in matches" :key="i" class="border-t border-neutral-800 align-top">
                      ...
                      <td class="px-3 py-2">
                        <div class="flex flex-wrap gap-1.5">
                          <Badge v-for="(g, gi) in m.groups" :key="gi" variant="outline" class="border-neutral-700 bg-neutral-900 text-[11px] text-neutral-300">
                    ...
                    <tr v-if="matches.length===0">
                      <td colspan="4" class="px-3 py-6 text-center text-neutral-500">No matches</td>
                    </tr>
                    </tbody>
                  </table>
                </div>
...
function run(){
  ...
  const re = buildRegex()
  if (!re || !text.value) { highlighted.value = escapeHtml(text.value || ''); return }
  ...
  if (replaceMode.value) {
    try { replaced.value = src.replace(re, replacement.value) }
    catch { replaced.value = '' }
  }
}
...
async function copyResult(){
  const payload = replaceMode.value ? replaced.value : matches.value.map((m,i)=>`${i+1}. "${m.value}" @ ${m.index}`).join('\n')
